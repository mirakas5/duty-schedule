...
def assign(
    ...
    Args:
        ...
        seed:    동점(점수 동일) 인원의 순서를 정하는 랜덤 시드.
                 None이면 매 호출마다 무작위(자동생성 때마다 순서가 달라짐).
                 값을 주면 재현 가능. 공정성(총량·간격)은 점수가 결정하므로
                 랜덤은 '동점자 순서'에만 영향 → 편차 ≤1·간격 균일은 그대로 보장.
    ...
    # 동점자 순서용 랜덤 키 (seed=None이면 매번 무작위)
    rng = random.Random(seed)
    rand_rank = {m.id: rng.random() for m in members}
    ...
    for pos, key in enumerate(week_keys):
        ...
        ranked = sorted(
            ...
            key=lambda m: (
                ...
                state[m.id].total,
                state[m.id].last_idx,
                rand_rank[m.id],  # 동점은 랜덤 (엑셀 순서 고정 방지)
            ),
        )
